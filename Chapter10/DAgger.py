...
def expert():
    ...
    sess_expert = tf.compat.v1.Session(graph=graph)

    saver = tf.compat.v1.train.import_meta_graph('expert/model.ckpt.meta')
...
def DAgger(hidden_sizes=[32,32], dagger_iterations=20, p_lr=1e-3, step_iterations=1000, batch_size=128, train_epochs=20, obs_dim=8, act_dim=2):
    ...
    act_max = tf.math.argmax(input=p_logits, axis=1)
    act_onehot = tf.one_hot(act_ph, depth=act_dim)

    # softmax cross entropy loss
    p_loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(labels=act_onehot, logits=p_logits))
    # Adam optimizer
    ...
    file_writer = tf.compat.v1.summary.FileWriter('log_dir/FlappyBird/DAgger_'+clock_time, tf.compat.v1.get_default_graph())
