...
class DQN_optimization:
    ...
    def __build_graph(self):
        ...
        with self.g.as_default():
            ...
            self.obs_ph = tf.compat.v1.placeholder(shape=(None, self.obs_dim[0]), dtype=tf.float32, name='obs')
            self.act_ph = tf.compat.v1.placeholder(shape=(None,), dtype=tf.int32, name='act')
            self.y_ph = tf.compat.v1.placeholder(shape=(None,), dtype=tf.float32, name='y')
            ...
            q_values = tf.reduce_sum(input_tensor=act_onehot * self.online_qv, axis=1)
            
            # MSE loss function
            self.v_loss = tf.reduce_mean(input_tensor=(self.y_ph - q_values)**2)
            # Adam optimize that minimize the loss v_loss
            self.v_opt = tf.compat.v1.train.AdamOptimizer(self.lr).minimize(self.v_loss)
...
def ESBAS(env_name, hidden_sizes=[32], lr=1e-2, num_epochs=2000, buffer_size=100000, discount=0.99, render_cycle=100, update_target_net=1000, 
    ...
    file_writer = tf.compat.v1.summary.FileWriter(LOG_DIR+'/ESBAS_'+clock_time+'_'+hyp_str, tf.compat.v1.get_default_graph())
