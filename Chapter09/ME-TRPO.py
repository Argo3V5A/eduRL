...
def softmax_entropy(logits):
    ...
    Softmax Entropy
    '''
    return -tf.reduce_sum(input_tensor=tf.nn.softmax(logits, axis=-1) * tf.nn.log_softmax(logits, axis=-1), axis=-1)
...
def gaussian_log_likelihood(ac, mean, log_std):
    ...
    return -0.5 * tf.reduce_sum(input_tensor=log_p, axis=-1)
...
def gaussian_DKL(mu_q, log_std_q, mu_p, log_std_p):
    ...
    '''
    return tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=0.5 * (log_std_p - log_std_q + tf.exp(log_std_q - log_std_p) + (mu_q - mu_p)**2 / tf.exp(log_std_p) - 1), axis=1))
...
def restore_model(old_model_variables, m_variables):    
    ...
    for m_v in m_variables:
        upd_m_rsh = tf.reshape(old_model_variables[it_v2 : it_v2+tf.reduce_prod(input_tensor=m_v.shape)], shape=m_v.shape)
        restore_m_params.append(m_v.assign(upd_m_rsh)) 
        it_v2 += tf.reduce_prod(input_tensor=m_v.shape)
...
def METRPO(env_name, hidden_sizes=[32], cr_lr=5e-3, num_epochs=50, gamma=0.99, lam=0.95, number_envs=1, 
    ...
    old_log_std_ph = tf.compat.v1.placeholder(shape=(act_dim), dtype=tf.float32, name='old_log_std')
    ...
    old_model_variables = tf.compat.v1.placeholder(shape=(None,), dtype=tf.float32, name='old_model_variables')
    ...
    with tf.compat.v1.variable_scope('actor_nn'):
        ...
        log_std = tf.compat.v1.get_variable(name='log_std', initializer=np.ones(act_dim, dtype=np.float32))
    ...
    p_noisy = p_means + tf.random.normal(tf.shape(input=p_means), 0, 1) * tf.exp(log_std)
    ...
    diverg = tf.reduce_mean(input_tensor=tf.exp(old_p_log_ph - p_log))
    
    # ratio
    ratio_new_old = tf.exp(p_log - old_p_log_ph)
    # TRPO surrogate loss function
    p_loss = - tf.reduce_mean(input_tensor=ratio_new_old * adv_ph)

    # MSE loss function
    v_loss = tf.reduce_mean(input_tensor=(ret_ph - s_values)**2)
    # Critic optimization
    ...
    p_grads = tf.gradients(ys=p_loss, xs=p_variables)
    p_grads_flatten = flatten_list(p_grads)

    ########### RESTORE ACTOR PARAMETERS ###########
    p_old_variables = tf.compat.v1.placeholder(shape=(None,), dtype=tf.float32, name='p_old_variables')
    # variable used as index for restoring the actor's parameters
    it_v1 = tf.Variable(0, trainable=False)
    restore_params = []

    for p_v in p_variables:
        upd_rsh = tf.reshape(p_old_variables[it_v1 : it_v1+tf.reduce_prod(input_tensor=p_v.shape)], shape=p_v.shape)
        restore_params.append(p_v.assign(upd_rsh)) 
        it_v1 += tf.reduce_prod(input_tensor=p_v.shape)
    ...
    dkl_diverg_grad = tf.gradients(ys=dkl_diverg, xs=p_variables) 

    dkl_matrix_product = tf.reduce_sum(input_tensor=flatten_list(dkl_diverg_grad) * p_ph)
    ...
    Fx = flatten_list(tf.gradients(ys=dkl_matrix_product, xs=p_variables))
    ...
    for p_v in p_variables:
        upd_rsh = tf.reshape(trpo_update[it_v : it_v+tf.reduce_prod(input_tensor=p_v.shape)], shape=p_v.shape)
        p_opt.append(p_v.assign_sub(upd_rsh))
        it_v += tf.reduce_prod(input_tensor=p_v.shape)
    ...
    for i in range(num_ensemble_models):
        ...
        m_loss = tf.reduce_mean(input_tensor=(nobs_ph - nobs_pred)**2)
    ...
    tf.compat.v1.summary.scalar('ratio_new_old',tf.reduce_mean(input_tensor=ratio_new_old), collections=['train'])
    ...
    tf.compat.v1.summary.scalar('p_std_mn', tf.reduce_mean(input_tensor=tf.exp(log_std)), collections=['train'])
    tf.compat.v1.summary.scalar('s_values_mn', tf.reduce_mean(input_tensor=s_values), collections=['train'])
    ...
    file_writer = tf.compat.v1.summary.FileWriter('log_dir/'+env_name+'/'+algorithm+'_'+clock_time+'_'+hyp_str, tf.compat.v1.get_default_graph())
