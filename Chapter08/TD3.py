...
def deterministic_actor_double_critic(x, a, hidden_sizes, act_dim, max_act=1):
    ...
    with tf.compat.v1.variable_scope('q1_mlp', reuse=True): # Use the weights of the mlp just defined
        q1_a = mlp(tf.concat([x,a], axis=-1), hidden_sizes, 1, last_activation=None)

    # Second critic
    with tf.compat.v1.variable_scope('q2_mlp'):
...
def TD3(env_name, hidden_sizes=[32], ac_lr=1e-2, cr_lr=1e-2, num_epochs=2000, buffer_size=5000, discount=0.99, render_cycle=10000, mean_summaries_steps=1000, 
    ...
    # Critics loss (MSE)
    q1_loss = tf.reduce_mean(input_tensor=(qa1_onl - y_ph)**2) 
    q2_loss = tf.reduce_mean(input_tensor=(qa2_onl - y_ph)**2)

    # Actor loss
    p_loss = -tf.reduce_mean(input_tensor=qd1_onl)
    ...
    # Set TensorBoard
    tf.compat.v1.summary.scalar('loss/q1', q1_loss)
    tf.compat.v1.summary.scalar('loss/q2', q2_loss)
    tf.compat.v1.summary.scalar('loss/p', p_loss)
    scalar_summary = tf.compat.v1.summary.merge_all()

    hyp_str = '-aclr_'+str(ac_lr)+'-crlr_'+str(cr_lr)+'-tau_'+str(tau)

    file_writer = tf.compat.v1.summary.FileWriter('log_dir/'+env_name+'/TD3_'+clock_time+'_'+hyp_str, tf.compat.v1.get_default_graph())
