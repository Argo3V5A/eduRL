...
def DDPG(env_name, hidden_sizes=[32], ac_lr=1e-2, cr_lr=1e-2, num_epochs=2000, buffer_size=5000, discount=0.99, render_cycle=100, mean_summaries_steps=1000, 
    ...
    # Critic loss (MSE)
    q_loss = tf.reduce_mean(input_tensor=(qa_onl - y_ph)**2) 
    # Actor loss
    p_loss = -tf.reduce_mean(input_tensor=qd_onl)
    ...
    # Set TensorBoard
    tf.compat.v1.summary.scalar('loss/q', q_loss)
    ...
    file_writer = tf.compat.v1.summary.FileWriter('log_dir/'+env_name+'/DDPG_'+clock_time+'_'+hyp_str, tf.compat.v1.get_default_graph())
