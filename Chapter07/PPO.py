...
def clipped_surrogate_obj(new_p, old_p, adv, eps):
    ...
    return -tf.reduce_mean(input_tensor=tf.minimum(rt*adv, tf.clip_by_value(rt, 1-eps, 1+eps)*adv))
...
def gaussian_log_likelihood(x, mean, log_std):
    ...
    return tf.reduce_sum(input_tensor=log_p, axis=-1)
...
def PPO(env_name, hidden_sizes=[32], cr_lr=5e-3, ac_lr=5e-3, num_epochs=50, minibatch_size=5000, gamma=0.99, lam=0.95, number_envs=1, eps=0.1, 
    ...
    if action_type == 'Discrete':
        ...
        act_smp = tf.squeeze(tf.random.categorical(logits=p_logits, num_samples=1))
        act_onehot = tf.one_hot(act_ph, depth=act_dim)
        p_log = tf.reduce_sum(input_tensor=act_onehot * tf.nn.log_softmax(p_logits), axis=-1)
    ...
    # Computational graph for the policy in case of a continuous action space
        ...
        p_noisy = p_logits + tf.random.normal(tf.shape(input=p_logits), 0, 1) * tf.exp(log_std)
    ...
    tf.compat.v1.summary.scalar('s_values_m', tf.reduce_mean(input_tensor=s_values), collections=['train'])

    if action_type == 'Box':
        tf.compat.v1.summary.scalar('p_std', tf.reduce_mean(input_tensor=tf.exp(log_std)), collections=['train'])
    ...
    file_writer = tf.compat.v1.summary.FileWriter('log_dir/'+env_name+'/PPO_'+clock_time+'_'+hyp_str, tf.compat.v1.get_default_graph())
