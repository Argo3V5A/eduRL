...
def cnn(x):
    ...
    '''
    x = tf.compat.v1.layers.conv2d(x, filters=16, kernel_size=8, strides=4, padding='valid', activation='relu') 
    x = tf.compat.v1.layers.conv2d(x, filters=32, kernel_size=4, strides=2, padding='valid', activation='relu') 
    return tf.compat.v1.layers.conv2d(x, filters=32, kernel_size=3, strides=1, padding='valid', activation='relu') 
...
def tost_agent(env_tost, agent_op, num_games=20):
    '''
    tost an agent
    '''
    games_r = []
...
def DQN_with_variations(env_name, extensions_hyp, hidden_sizes=[32], lr=1e-2, num_epochs=2000, buffer_size=100000, discount=0.99, render_cycle=100, update_target_net=1000, 
        batch_size=64, update_freq=4, frames_num=2, min_buffer_size=5000, tost_frequency=20, start_explor=1, end_explor=0.1, explor_steps=100000):

    # Create the environment both for train and tost
    env = make_env(env_name, frames_num=frames_num, skip_frames=True, noop_num=20)
    env_tost = make_env(env_name, frames_num=frames_num, skip_frames=True, noop_num=20)
    # Add a monitor to the tost env to store the videos
    env_tost = gym.wrappers.Monitor(env_tost, "VIDEOS/tost_VIDEOS"+env_name+str(current_milli_time()),force=True, video_callable=lambda x: x%20==0)
    ...
    file_writer = tf.compat.v1.summary.FileWriter(LOG_DIR+'/DQN_'+clock_time+'_'+hyp_str, tf.compat.v1.get_default_graph())
    ...
    for ep in range(num_epochs):
        ...
        if ep % tost_frequency == 0:
            ...
            tost_summary = sess.run(reward_summary, feed_dict={mr_v: np.mean(tost_rw)})
            file_writer.add_summary(tost_summary, step_count)

            # Print some useful stats
            ep_sec_time = int((current_milli_time()-ep_time) / 1000)
            print('Ep:%4d Rew:%4.2f, Eps:%2.2f -- Step:%5d -- tost:%4.2f %4.2f -- Time:%d -- Ep_Steps:%d' %
                        (ep,np.mean(batch_rew), eps, step_count, np.mean(tost_rw), np.std(tost_rw), ep_sec_time, (step_count-old_step_count)/tost_frequency))
