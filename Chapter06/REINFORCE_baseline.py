...
def REINFORCE_baseline(env_name, hidden_sizes=[32], p_lr=5e-3, vf_lr=8e-3, gamma=0.99, steps_per_epoch=100, num_epochs=1000):
    ...
    act_multn = tf.squeeze(tf.random.categorical(logits=p_logits, num_samples=1))
    actions_mask = tf.one_hot(act_ph, depth=act_dim)
    p_log = tf.reduce_sum(input_tensor=actions_mask * tf.nn.log_softmax(p_logits), axis=1)
    # entropy useful to study the algorithms
    entropy = -tf.reduce_mean(input_tensor=softmax_entropy(p_logits))
    p_loss = -tf.reduce_mean(input_tensor=p_log*ret_ph)
    ...
    # MSE loss function
    v_loss = tf.reduce_mean(input_tensor=(rtg_ph - s_values)**2)
    ...
    tf.compat.v1.summary.scalar('s_values', tf.reduce_mean(input_tensor=s_values), collections=['train'])
    ...
    file_writer = tf.compat.v1.summary.FileWriter('log_dir/{}/REINFORCE_basel_{}_{}'.format(env_name, clock_time, hyp_str), tf.compat.v1.get_default_graph())
