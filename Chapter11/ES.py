...
def worker(env_name, initial_seed, hidden_sizes, lr, std_noise, indiv_per_worker, worker_name, params_queue, output_queue):
    ...
    tf.compat.v1.random.set_random_seed(initial_seed)
    np.random.seed(initial_seed)
    

    with tf.device("/cpu:" + worker_name):
        
        obs_ph = tf.compat.v1.placeholder(shape=(None, obs_dim), dtype=tf.float32, name='obs_ph')
        new_weights_ph = tf.compat.v1.placeholder(shape=(None,), dtype=tf.float32, name='new_weights_ph')
        ...
        for a_v in agent_variables:
            upd_rsh = tf.reshape(new_weights_ph[it_v1 : it_v1+tf.reduce_prod(input_tensor=a_v.shape)], shape=a_v.shape)
            update_weights.append(a_v.assign(upd_rsh))
            it_v1 += tf.reduce_prod(input_tensor=a_v.shape)
        ...
        vars_grads_list = []
        for a_v in agent_variables:
            vars_grads_list.append(tf.reshape(new_weights_ph[it_v2 : it_v2+tf.reduce_prod(input_tensor=a_v.shape)], shape=a_v.shape))
            it_v2 += tf.reduce_prod(input_tensor=a_v.shape)
...
def ES(env_name, hidden_sizes=[8,8], number_iter=1000, num_workers=4, lr=0.01, indiv_per_worker=10, std_noise=0.01):
    ...
    file_writer = tf.compat.v1.summary.FileWriter('log_dir/'+env_name+'/'+clock_time+'_'+hyp_str, tf.compat.v1.get_default_graph())
